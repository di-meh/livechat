...
function isGuildMember(member: ChatInputCommandInteraction['member']): member is GuildMember {
...
export async function createLiveChatUser(
	...
	if (interaction.inCachedGuild()) {
		...
		const guildUser = await interaction.guild.members.fetch(interaction.user.id).catch(() => null)
...
export function getTargetDescription(target: LiveChatTarget, targetUser: User | null): string {
